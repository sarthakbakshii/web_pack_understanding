# Web Pack basic understanding

> **Visit inside [scratch_example](https://github.com/sarthakbakshii/web_pack_understanding/tree/main/scratch_example "scratch_example") to know how to do basic instalation and working, after reading that this readme file will make sence.**
...
2. prevents the pollution of global scope.

**Q2) Definition in easy word:**
...
- When you make changes to your code, Webpack Dev Server automatically rebuilds your project and refreshes the browser, saving you from manual refreshes.
- Hot Module Replacement (HMR) allows parts of the application to be updated without a full page reload.

2. In-Memory Compilation:

- Webpack Dev Server serves the bundled files from memory rather than from the file system. This makes the build and serve process faster since it doesn't need to write files to disk.

3. Development Proxy:

- It allows you to proxy requests from your development environment to an API server, making it easier to handle backend and frontend development in parallel.

4. Customizable Configuration:

- You can customize its configuration to serve your files, set up HMR, use specific ports, or handle more complex setups like proxying API requests.
